...
def test_WorkActivity_get_all_activity_list(app, client, users, db_register_full_action):
    with app.test_request_context():
        login_user(users[2]["obj"])
        activity = WorkActivity()
...
def test_WorkActivity_get_activity_action_role(app, activity_with_roles, action_id, action_order, expected_index, expected_included):
    ...
    _, users = workflow_activity.get_activity_action_role(activity.activity_id, action_id, action_order)
    print(users)
